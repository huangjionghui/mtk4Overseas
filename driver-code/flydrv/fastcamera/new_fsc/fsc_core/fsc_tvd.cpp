...
//extern "C" HANDLE      	  g_hEventBackSig        ;//    = NULL; //modify by jdp 20120908
...
static BOOL _TvdControl(UINT32 CtrlCode)
{
	...
	if ((CtrlCode == TVD_CONTROL_CODE_INIT) && (g_FscSyncTvdStat != TVD_STAT_INIT))
	{
		...
		g_TvdDrvInitPara.eVdoOutFmt  = TVD_VDOFMT_YUV420;
 		...
 		g_TvdDrvInitPara.u4CHACvbsInxP = CVBSIN_3P;//CVBSIN_1P modify by jdp 20120821
...
extern "C" BOOL  g_bBackCarFlagEnable;
...
DWORD WINAPI _TvdListenStatThread(LPVOID lpPara)
{
	...
	RETAILMSG(1,(TEXT("[Back-fsc_tvd.cpp] _TvdListenStatThread.............\r\n")));
	...
	while(!g_TvdListenThreadExit)
	{
		...
		case TVD_SIG_OFF_FLAG:	//ghTvdLossImageEvent
			...
			{	
				OnImgSignalLoss(0);
				SendBackStatusToDriver(2);//有倒车，没视频
				RETAILMSG(1 , (TEXT("\r\n[Back-fsc_tvd.cpp]:Back Wince Action:1,Video:0\r\n")));
			}
			break;

		case TVD_SIG_ON_FLAG:	//ghTvdGetImageEvent
			OnImgSignalGet(dwEvtData & (~TVD_SIG_STATE_MASK));
			SendBackStatusToDriver(3);//有倒车，有视频
			RETAILMSG(1 , (TEXT("\r\n[Back-fsc_tvd.cpp]:Back Wince Action:1,Video:1\r\n")));
			break;
		case TVD_MODE_CHANGE_FLAG: //ghTvdSysEvent
			OnImgSysChange(dwEvtData & (~TVD_SIG_STATE_MASK));
			RETAILMSG(1, (TEXT("[Back-fsc_tvd.cpp]by jdp  TVD_MODE_CHANGE_FLAG\n"))); 
			break;
		default: //error 
			RETAILMSG(1,(TEXT("[Back-fsc_tvd.cpp]***tvd error event!!***\n\r")));
			break;
		}
...
BOOL  TvdPreInit(HWND hAppWnd)
{
 ...
 //    	g_hEventBackSig = OpenEvent(EVENT_ALL_ACCESS , FALSE , TEXT("FLY_BACK_CAMERA_ON_OFF"));//OpenEvent(EVENT_ALL_ACCESS, FALSE, TEXT("SYSTEM/BackCarAppReady"));
	...
	//}
	 g_FscTvdInited = TRUE;
	 RETAILMSG(1,(TEXT("OWL_DEBUG:[%s]---------------------------bRet = %d\r\n"),__FUNCTION__,bRet));
EXIT:
	 return bRet;
}
...
BOOL TvdSignalIsOn(VOID)
{
    if (g_FscTvdInited)
	{
		...
		RETAILMSG(1, (L"[TVD_TEST] Control Tvd Driver : Get tvd signal status:%d\r\n"),g_TvdSigOn.fgSigOn);
